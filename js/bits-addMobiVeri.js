...
function changedConfCode(e) {
    $(".inp-code").html('<div class="preloader-wrapper small active" style=" width: 25px; height: 25px; margin-top: 5px;"> <div class="spinner-layer spinner-green-only"> <div class="circle-clipper left"> <div class="circle"></div></div><div class="gap-patch"> <div class="circle"></div></div><div class="circle-clipper right"> <div class="circle"></div></div></div></div>'), doFetch({
        ...
        $(".inp-code").html("ok"), "ok" == e.status ? ($("#inp-phone").prop("disabled", !1), $("#inp-code").prop("disabled", !0), M.toast({
            html: "Phone Number added"
        }), localStorage.setItem('userVerifiedNumber', true), M.Modal.init(document.getElementById("MobileModal"), {}).close()) : console.log(e)
    })
}
